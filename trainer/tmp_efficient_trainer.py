...
class EfficientTrainer(Trainer):
    ...
    def train(self,resume_from_checkpoint):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))): #! 20 epoch
            ...
            self.alter_stage = False
            for step, inputs in enumerate(epoch_iterator):
                self.alter_stage = self.global_step % 100 < 50
    ...
    def training_step(self, model: torch.nn.Module, distill: bool, inputs: Dict[str, Union[torch.Tensor, Any]]) -> List[torch.Tensor]:
        model.train()
        if self.l0_module is not None and not self.alter_stage:
            self.l0_module.train()
        inputs = self._prepare_inputs(inputs)

        distill_loss = None
        distill_ce_loss = None
        if distill and not self.alter_stage:
            with torch.no_grad():
                ...
                teacher_zs, lora_weights = None, None
                if self.additional_args.do_iterative_distill:
                    teacher_zs_lora_weights = self.iter_manager.check_use_iterative_distill(self.t_total - self.global_step)
                    if teacher_zs_lora_weights is not None:
                        teacher_zs, lora_weights = teacher_zs_lora_weights
                    if teacher_zs is not None:
                        self.fill_inputs_with_zs(teacher_zs, teacher_inputs)
                print()
                print('check lora weights for student before backup')
                for n, m in model.named_parameters():
                    if 'lora_' in n:
                        print(m.data.max().item(),m.data.min().item(),m.data.mean().item())
                        break
                # backup student lora weights
                ...
                if lora_weights is not None:
                    for n, m in model.named_parameters():
                        if 'lora_' in n:
                            gather = lora.should_gather(m)
                            with gather:
                                m.data = lora_weights[n.replace('module.','')].to(m.device)
                else:
                    for n, m in model.named_parameters():
                        if 'lora_' in n:
                            gather = lora.should_gather(m)
                            with gather:
                                m.data *= 0
                print()
                print('check lora weights for this teacher')
                for n, m in model.named_parameters():
                    if 'lora_' in n:
                        print(m.data.max().item(),m.data.min().item(),m.data.mean().item())
                        break
                teacher_outputs = model(**teacher_inputs)
                # restore model by student_lora_weights
                for n, m in model.named_parameters():
                    if 'lora_' in n:
                        gather = lora.should_gather(m)
                        with gather:
            ...
            print()
            print('check lora weights for this student')
            for n, m in model.named_parameters():
                if 'lora_' in n:
                    print(m.data.max().item(),m.data.min().item(),m.data.mean().item())
                    break
