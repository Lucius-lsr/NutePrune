...
class MistralMLP(nn.Module):
    def __init__(self, config):
        ...
        if should_apply_lora(config, "ffn"):
            import utils.lora_utils as lora
            self.gate_proj = lora.Linear(self.hidden_size, self.intermediate_size, r=config.lora_rank,
                                         lora_alpha=config.lora_alpha, bias=False)
            self.down_proj = lora.Linear(self.intermediate_size, self.hidden_size, r=config.lora_rank,
                                         lora_alpha=config.lora_alpha, bias=False)
            self.up_proj = lora.Linear(self.hidden_size, self.intermediate_size, r=config.lora_rank, lora_alpha=config.lora_alpha,
                                       bias=False)
        else:
    ...
    def forward(self, x, intermediate_z=None):
...
def should_apply_lora(config, where: str) -> bool:
...
class MistralAttention(nn.Module):
    ...
    def __init__(self, config: MistralConfig, layer_idx: Optional[int] = None):
        ...
        if config.use_lora and "V" in lora_param_set:
            ...
            self.v_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=False)
        if should_apply_lora(config, "attention_output"):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if hidden_z is not None:
            ...
            hidden_states = hidden_states.mul(hidden_z).to(ori_dtype)
        hidden_states = residual + hidden_states

        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states = self.mlp(hidden_states, intermediate_z=intermediate_z)
        if hidden_z is not None:
            ori_dtype = hidden_states.dtype
            hidden_states = hidden_states.mul(hidden_z).to(ori_dtype)
        hidden_states = residual + hidden_states
...
class MistralModel(MistralPreTrainedModel):
    ...
    def forward(
        ...
        if hidden_z is not None:
            ori_dtype = inputs_embeds.dtype
            inputs_embeds = inputs_embeds.mul(hidden_z).to(ori_dtype)
